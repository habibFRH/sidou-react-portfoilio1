...
function NavBar() {
  ...
  return (
    <div className='flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed z-10'>
        <div>
            <h1 className='text-5xl font-signature ml-2 cursor-default'>SidouFrh</h1>
        </div>
        ...
        {nav && (
            ...
            bg-gradient-to-b from-black to-gray-800 text-gray-500'>
                {links.map(({id , link})=>(
                    <li key={id} className='px-4 cursor-pointer capitalize my-6 text-4xl hover:text-white duration-300'>
